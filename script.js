 ...
 function showChatInterface() {
     ...
     if (currentUser && users.hasOwnProperty(currentUser)) {
         ...
         updateContactList(Object.keys(users).filter(user => user !== currentUser)); // Mantém a lista de outros usuários para convidados (para visualização?)
     }
 }
...
function showContactList() {
 ...
 loginButton.addEventListener('click', () => {
     ...
     console.log('Tentativa de login:', username, password); // Adicionado para depuração

     if (validateUser(username, password)) { // Usando a função de validação de user.js
         console.log('Login bem-sucedido para:', username); // Adicionado para depuração
         currentUser = username;
         localStorage.setItem('currentUser', currentUser);
         showChatInterface();
         loadActiveChats(); // Carrega os contatos privados após o login
     } else {
         console.log('Falha no login para:', username); // Adicionado para depuração
...
backToContactsFromGroupsButton.addEventListener('click', () => {
 ...
 guestLoginButton.addEventListener('click', () => {
     ...
     if (guestName) {
         ...
         localStorage.setItem('currentUser', currentUser);
         showChatInterface();
         // updateContactList([]); // Removido conforme a correção anterior
 ...
 if (currentUser && users.hasOwnProperty(currentUser)) {
     ...
     updateContactList([]); // Inicializa com lista vazia para não logados
 }
